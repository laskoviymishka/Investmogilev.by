...
namespace BusinessLogic.Notification
{
    public class PortalMessageHandler
    {
        ...
        {
            return RepositoryContext.Current.All<MessageQueue>(m => (m.To == userName || m.Cc.Contains(userName)) && m.IsSended && m.Type == MessageType.Portal);
        }

        public IEnumerable<MessageQueue> Unread(string userName)
        {
            return RepositoryContext.Current.All<MessageQueue>(m => (m.To == userName || m.Cc.Contains(userName)) && !m.IsReaded && m.IsSended && m.Type == MessageType.Portal);
        }
        ...
        {
            return RepositoryContext.Current.All<MessageQueue>(m => (m.From == userName) && !m.IsReaded && m.IsSended && m.Type == MessageType.Portal);
        }

        public IEnumerable<MessageQueue> Sended(string userName)
        {
            return RepositoryContext.Current.All<MessageQueue>(m => (m.From == userName) && m.IsSended && m.Type == MessageType.Portal);
        }

        public MessageQueue Message(string userName, string id)
        {
            return RepositoryContext.Current.GetOne<MessageQueue>(m => m.From == userName && m._id == id && m.Type == MessageType.Portal);
        }
        ...
        {
            var message = Message(userName, messageId);
            if (message == null)
            {
