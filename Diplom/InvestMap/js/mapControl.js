...
function openPopUp(e,type) {
    ...
    $('#popupContainer')
    linkToDetails = 'http://localhost:2205/InvestProjects/PopUpDetails' + type.toString() + '/' + e.toString()
    $.ajax({
        url: linkToDetails,
        dataType: 'html',
...
$(function () {
    $.ajax({
        url: 'http://localhost:2205/InvestProjects/ListOfProjects?callback=jQuery110205763436721172184_1381677318690&_=1381677318691',
        dataType: 'html',
        success: function (data) {
...
function getProjectGeoData() {
    $.ajax({
        url: 'http://localhost:2205/InvestProjects/ProjectGeoJSON',
        type: "GET",
        dataType: "json",
        success: function (data) {
            addMarkers(data);
...
function addMarkers(data){
    ...
    for (var i = 0; i < projects.length; i++) {
        ...
        google.maps.event.addListener(marker, 'click', function () {
            for (var i = 0; i < projects.length; i++) {
                if (new google.maps.LatLng(projects[i].Lat, projects[i].Lng).toString() == marker.getPosition().toString()) {
